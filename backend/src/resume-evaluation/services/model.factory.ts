...
class MockStructuredModel {
  ...
  withStructuredOutput(schema: any) { return { invoke: (prompt: string) => this.llm.generateStructured({ system: 'You are a deterministic test model. Return data matching the requested schema.', prompt, schema }) }; }
}
